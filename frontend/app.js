...
var RENDERERS = window.RENDERERS = {
  ...
  'it-companies': function() { return typeof renderITCompaniesModule === 'function' ? renderITCompaniesModule() : buildPlaceholderPage('it-companies'); },
  'consultancies': function() { return typeof renderConsultanciesModule === 'function' ? renderConsultanciesModule() : buildPlaceholderPage('consultancies'); },
  ...
  'rc-candidates': function() { return typeof rcRenderPageWithHeader === 'function' ? rcRenderPageWithHeader('candidates') : buildPlaceholderPage('rc-candidates'); },
...
function navigateEntity(id) {
  switch(id) {
    case 1: navigate('employees'); break;
    case 3: navigate('academy-trainees'); break;
    case 8: navigate('rc-dashboard'); break;
    default: showNotification('Module not available in prototype','info'); break;
  }
}
...
function doDelete(listKey, id, msg) {
  const item = (MOCK[listKey] || []).find(x => String(x.id) === String(id));
  if (item && item.dbId && window.API) {
    try {
      if (listKey === 'employees' || listKey === 'clients') {
        window.API.delete('/users/' + item.dbId).catch(() => {});
      } else if (listKey === 'projects') {
        window.API.delete('/projects/' + item.dbId).catch(() => {});
      }
    } catch (err) {
...
async function doLogin(e) {
  ...
  if (window.API) {
    try {
      ...
      console.warn('Backend API login skipped, using mock fallback:', apiErr.message);
    }
  }

  if (!user) {
    user = MOCK.admin.find(a => a.username === u && a.password === p);
  ...
  document.getElementById('topbar-user-name').textContent = (user.name || user.username || 'User').split(' ')[0];
  document.getElementById('topbar-user-role').textContent = user.roles && user.roles.length ? user.roles[0].role : (user.role || 'Super Admin');
  document.getElementById('topbar-user-avatar').textContent = (user.name || user.username || 'U').charAt(0).toUpperCase();
  buildSidebar();
  navigate('dashboard');
...
async function submitEmployee() {
  ...
  const base = {
    ...
    payroll: v.payroll || '0',
    regNo: Number(v.regNo) || 0,
    joinDate: v.joinDate,
    status: (emp ? emp.status : 'Active')
  };
  base.companyEmail = v.personalEmail || (v.firstName.toLowerCase() + '@roririsoft.com');
  ...
  if (window.API) {
    try {
      const payload = {
        ...
        phone: base.phone,
        email: base.personalEmail || base.companyEmail,
      ...
      if (emp && emp.dbId) {
        const res = await window.API.put('/users/' + emp.dbId, payload);
        if (res && res.data) base.dbId = res.data.id;
      } else {
        const res = await window.API.post('/users', payload);
        if (res && res.data) {
          base.dbId = res.data.id;
          base.id = 'E' + String(res.data.id).padStart(3, '0');
        }
      }
    } catch (apiErr) {
      console.warn('Backend API save warning:', apiErr.message);
    }
  }

  if (emp) Object.assign(emp, base);
  else {
    if (!base.id) base.id = genId(MOCK.employees, 'E');
    MOCK.employees.push(base);
  }
  saveAdminDB();
  syncPortalsToStorage();
  closeModal();
  navigate('employees');
  showNotification('Employee saved and stored in PostgreSQL!', 'success');
}
...
async function submitClient() {
  ...
  if (window.API) {
    try {
      ...
      if (c && c.dbId) {
        await window.API.put('/users/' + c.dbId, payload);
      } else {
        const res = await window.API.post('/users', payload);
        if (res && res.data) {
          base.dbId = res.data.id;
          base.id = 'C' + String(res.data.id).padStart(3, '0');
        }
      }
    } catch (apiErr) {
      console.warn('API client sync warning:', apiErr.message);
    }
  }

  if (c) Object.assign(c, base);
  else {
    if (!base.id) base.id = genId(MOCK.clients, 'C');
    MOCK.clients.push(base);
  }
  saveAdminDB();
  syncPortalsToStorage();
  closeModal();
  navigate('clients');
  showNotification('Client saved and stored in PostgreSQL!', 'success');
}
  ...
  if (window.API) {
    try {
      const payload = {
        ...
        status: base.status === 'Completed' ? 'delivered' : (base.status === 'In Progress' ? 'in_progress' : 'new'),
        payStatus: base.payStatus === 'Paid' ? 'paid' : (base.payStatus === 'Partially' ? 'partial' : 'pending')
      };
      if (existing && existing.dbId) {
        await window.API.put('/projects/' + existing.dbId, payload);
      } else {
        const res = await window.API.post('/projects', payload);
        if (res && res.data) {
          base.dbId = res.data.id;
          base.id = 'P' + String(res.data.id).padStart(3, '0');
        }
      }
    } catch (apiErr) {
      console.warn('API project sync warning:', apiErr.message);
    }
  }

  if (existing) Object.assign(existing, base);
  else {
    if (!base.id) base.id = genId(MOCK.projects, 'P');
    MOCK.projects.push(base);
  }
  saveAdminDB();
  syncPortalsToStorage();
  closeModal();
  navigate('projects');
  showNotification('Project saved and stored in PostgreSQL!', 'success');
}
...
// ============ BACKEND POSTGRESQL SYNC ============
async function syncEmployeesFromBackend() {
  if (!window.API) return;
  try {
    const res = await window.API.get('/users?type=employee');
    if (res && res.success && Array.isArray(res.data) && res.data.length > 0) {
      MOCK.employees = res.data.map(u => ({
        id: 'E' + String(u.id).padStart(3, '0'),
        dbId: u.id,
        firstName: u.firstName || (u.name ? u.name.split(' ')[0] : 'Emp'),
        lastName: u.lastName || (u.name ? u.name.split(' ').slice(1).join(' ') : ''),
        name: u.name,
        role: (u.roles && u.roles.length ? u.roles[0].role : 'Developer'),
        entity: (u.company ? u.company.name : 'RORIRI SOFTWARE SOLUTIONS'),
        phone: u.phone || '-',
        personalEmail: u.email || '-',
        companyEmail: u.email || `${(u.firstName || 'emp').toLowerCase()}@roririsoft.com`,
        status: u.status === 'active' ? 'Active' : 'Inactive',
        joinDate: u.createdAt ? new Date(u.createdAt).toISOString().split('T')[0] : '2024-04-29',
        dob: u.dob || '-',
        ...
        address: u.address || '-',
        username: u.username || '',
        payroll: u.payroll || '15,000',
        department: u.department ? u.department.name : 'Development',
        regNo: u.regNo || u.id
      }));
      saveAdminDB();
      syncPortalsToStorage();
      if (currentPage === 'employees') {
        ...
        if (tbody) navigate('employees');
      }
    }
  } catch (err) {
    console.warn('Could not sync employees from backend:', err.message);
  }
}

async function syncClientsFromBackend() {
  if (!window.API) return;
  try {
    const res = await window.API.get('/users?type=client');
    if (res && res.success && Array.isArray(res.data) && res.data.length > 0) {
      MOCK.clients = res.data.map(u => ({
        id: 'C' + String(u.id).padStart(3, '0'),
        dbId: u.id,
        name: u.name,
        company: u.companyTitle || (u.company ? u.company.name : 'Client Company'),
        location: u.location || '-',
        email: u.email || '-',
        phone: u.phone || '-',
        status: u.status === 'active' ? 'Active' : 'Inactive'
    ...
    console.warn('Could not sync clients from backend:', err.message);
  }
}
...
async function syncProjectsFromBackend() {
  ...
  try {
    ...
    if (res && res.success && Array.isArray(res.data) && res.data.length > 0) {
      MOCK.projects = res.data.map(p => ({
        id: 'P' + String(p.id).padStart(3, '0'),
        dbId: p.id,
        name: p.name,
        services: p.services || 'Web Development',
        tech: p.techStack || 'JavaScript, PHP',
        client: p.clientName || (p.clientUser ? p.clientUser.name : 'Client'),
        clientEmail: p.clientEmail || (p.clientUser ? p.clientUser.email : ''),
        address: p.clientUser ? p.clientUser.location : '',
        startDate: p.startDate ? new Date(p.startDate).toISOString().split('T')[0] : '2024-08-01',
        duration: p.duration || '6 months',
        amount: p.amount || 0,
        balance: p.balance || 0,
        description: p.description || '',
        status: p.status === 'delivered' ? 'Completed' : (p.status === 'in_progress' ? 'In Progress' : 'New'),
        payStatus: p.payStatus === 'paid' ? 'Paid' : (p.payStatus === 'partial' ? 'Partially' : 'Pending'),
        developers: (p.assignments || []).map(a => a.assigneeUser ? a.assigneeUser.name : '').filter(Boolean),
        payments: []
      }));
      saveAdminDB();
      syncPortalsToStorage();
    ...
    console.warn('Could not sync projects from backend:', err.message);
  }
}
...
async function initApp() {
  ...
  if (sessionStorage.getItem('roriri_loggedIn') === 'true') {
    ...
    if (savedUser) {
      ...
      document.getElementById('login-page').style.display = 'none';
      document.getElementById('app-page').style.display = 'flex';
      document.getElementById('topbar-user-name').textContent = (currentUser.name || currentUser.username || 'User').split(' ')[0];
      document.getElementById('topbar-user-role').textContent = currentUser.roles && currentUser.roles.length ? currentUser.roles[0].role : (currentUser.role || 'Super Admin');
      document.getElementById('topbar-user-avatar').textContent = (currentUser.name || currentUser.username || 'U').charAt(0).toUpperCase();
  ...
  // Live Database Sync on load
  await syncEmployeesFromBackend();
  await syncClientsFromBackend();
...
function renderITCompaniesModule() {
  return `
    <div class="page-header" style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1.25rem;flex-wrap:gap:1rem;">
      <div>
